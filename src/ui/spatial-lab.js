...
export function initSpatialLab(opts) {
  ...
  mainCanvas.addEventListener('click', (e) => {
    ...
    for (const key of layout.channels) {
      ...
      if (d < bestDist) { bestDist = d; hit = key; }
    }
    if (!hit) return;
    if (e.shiftKey || e.metaKey) {
      ...
      else { solo.clear(); solo.add(hit); }
    }
    syncGroupButtons();
    announce(`Speaker ${hit} ${solo.has(hit) ? 'solo' : 'unsoloed'} — monitoring only`);
  });
  ...
  const syncGroupButtons = () => {
    ...
    for (const btn of groupStrip.querySelectorAll('[data-group]')) {
      ...
      let pressed = false;
      if (solo.size) {
        for (const s of solo) if ((groups[g] && groups[g].includes(s)) || groupFor(s) === g) pressed = true;
      }
  ...
  $('#spatialSoloClear')?.addEventListener('click', () => { solo.clear(); syncGroupButtons(); });
  ...
  const setYaw = (deg) => {
    ...
    for (const b of orientBtns) b.setAttribute('aria-pressed', String(Number(b.dataset.yaw) === Math.round(yaw / 45) * 45 && Math.abs(Number(b.dataset.yaw) - yaw) < 1));
    // Update card selection strictly by exact match for front/left/right/rear
    for (const b of orientBtns) b.setAttribute('aria-pressed', String(Number(b.dataset.yaw) === yaw));
    if (orientHint) orientHint.textContent = `${yaw}° — facing ${yaw === 0 ? 'front' : yaw === 90 ? 'left' : yaw === -90 ? 'right' : yaw === 180 || yaw === -180 ? 'rear' : 'custom'}. Drag dial or use ←/→.`;
    // Store for future use (adapter boundary)
  ...
  if (dial) {
    ...
    dial.addEventListener('pointerdown', (e) => { dragging = true; dial.setPointerCapture(e.pointerId); setYaw(angleFrom(e)); });
    dial.addEventListener('pointermove', (e) => { if (dragging) setYaw(angleFrom(e)); });
    dial.addEventListener('pointerup', () => dragging = false);
    dial.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowLeft') { e.preventDefault(); setYaw(yaw + 15); }
      if (e.key === 'ArrowRight') { e.preventDefault(); setYaw(yaw - 15); }
      if (e.key === 'Home') { e.preventDefault(); setYaw(0); }
    });
  }
  ...
  const tick = () => {
    ...
    try { drawElevation(elevCanvas, { layoutId: lid }); } catch { void 0; }
    try { drawMotion(motionCanvas, motionMode); } catch { void 0; }
    try { energy.tick(); } catch { void 0; }
  };

  return { tick, get solo() { return solo; }, get yaw() { return yaw; } };
}

function announce(msg) {
  const r = document.getElementById('live-region');
  if (r) { r.textContent = msg; setTimeout(() => { if (r.textContent === msg) r.textContent = ''; }, 2000); }
}
